...
for fullname in glob.glob(f"pmlb/datasets/{sys.argv[1]}*"):
    ...
    for seed in [23654, 15795 ,860, 5390, 29802, 21575, 11964, 11284, 22118, 6265]:
        if os.path.exists(f'srbench/experiment/results_test/{dname}_{algname}_{seed}.json'):
            continue
...
python evaluate_model.py ~/pmlb/datasets/{dname}/{dname}.tsv.gz -ml {algname} -seed {seed}
''', file=fw)
        fw.close()
